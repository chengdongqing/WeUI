package top.chengdongqing.weui.components.loading
...
import weui_kmp.shared.generated.resources.ic_loading
...
@Composable
fun MiLoadingWeb(color: Color = GreenPrimary) {
    val infiniteTransition = rememberInfiniteTransition(label = "")
...
@Composable
fun MiLoadingMobile(
    borderColor: Color = WeTheme.colorScheme.textPrimary,
    ...
    Canvas(
        ...
        val dotX = cos(angle.value.toDouble().toRadians()) * circleRadius + center.x
        val dotY = sin(angle.value.toDouble().toRadians()) * circleRadius + center.y
...
@Composable
fun DotDanceLoading(color: Color = GreenPrimary) {
    val infiniteTransition = rememberInfiniteTransition(label = "")
