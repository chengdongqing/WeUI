...
import top.chengdongqing.weui.util.DefaultTimeFormatter
import top.chengdongqing.weui.util.localDate
import top.chengdongqing.weui.util.localTime
import kotlin.time.Clock

@Composable
fun PickerScreen(onBack: () -> Unit) {
    WeScreen(
        title = "Picker",
        description = "滚动选择器",
...
private fun TimePickDemo() {
    ...
    WeInput(
        value = value.format(DefaultTimeFormatter),
        textAlign = TextAlign.Center,
        disabled = true
    )
    Spacer(modifier = Modifier.height(20.dp))
...
private fun CarPickDemo() {
    ...
    val sourceMap = remember {
        arrayOf(
            "特斯拉" to listOf("Model 3", "Model Y", "Model S"),
